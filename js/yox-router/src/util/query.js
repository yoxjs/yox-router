 ...
 * 把 GET 参数解析成对象
 */
export function parse(Yox, query) {
    let result;
    Yox.array.each(query.split(constant.SEPARATOR_QUERY), function (term) {
        let terms = term.split(constant.SEPARATOR_PAIR), key = Yox.string.trim(terms[0]), value = terms[1];
        if (key) {
            if (!result) {
                result = {};
            }
 ...
 */
export function stringify(Yox, query) {
    const result = [];
        ...
        if (Yox.is.array(value)) {
            Yox.array.each(value, function (value) {
                ...
                if (Yox.is.string(str)) {
                    result.push(key + constant.FLAG_ARRAY + constant.SEPARATOR_PAIR + str);
                }
            });
        }
        else {
            const str = valueUtil.stringify(Yox, value);
            if (Yox.is.string(str)) {
                result.push(key + constant.SEPARATOR_PAIR + str);
            }
        }
    }
